...
database_list = ["track_data"]  # Enter databases to be tracked
...
@app.route("/dispense_food", methods=["GET", "POST"])
def dispense_food():
    global dispense_rn
    print("random stuff")
    ...
    if request.method == "POST":
        ...
        if jsondata["accessCode"] == button_accesscode:
            print("access code is correct")
            print(jsondata)
            if jsondata["command"] == "on":
                if dispense_rn == False:
                    dispense_rn = True
                    print("dispensing food")
                    data = {"dispense_rn": dispense_rn}
            elif jsondata["command"] == "off":
                if dispense_rn == True:
            ...
            data = {"dispense_rn": dispense_rn}
        data = {"dispense_rn": dispense_rn}
        response = make_response(json.dumps(data))  # Converting the data to json
